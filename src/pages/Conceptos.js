        ...
        <div className="h-screen bg-black overflow-hidden relative flex flex-col justify-center">
            ...
            <div className="max-w-[85rem] w-full mx-auto px-6 relative z-10">
                ...
                >
                    ...
                    <h1 className="text-3xl md:text-4xl lg:text-5xl font-inter font-semibold tracking-tight text-white leading-tight">
                        El lenguaje de los <span className="text-white">datos</span>.
                    </h1>
                </div>

                {/* DeepMind Typographic Split Layout */}
                <div 
                    className={`
                        w-full flex flex-col lg:flex-row gap-12 lg:gap-24 items-start
                ...
                >
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col gap-4 max-h-[500px] overflow-y-auto pr-4 custom-scrollbar">
                        {concepts.map((concept) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`
                                        text-left group flex items-center py-3 pr-4 my-1 rounded-r-lg
                                        ${isActive ? 'opacity-100 pl-4 border-l-2 border-[#034EA2] bg-white/10' : 'opacity-40 hover:opacity-80 border-l-2 border-transparent pl-2 hover:pl-4 transition-all duration-200'}
                                    `}
                                >
                                    <span className={`font-inter tracking-tight ${isActive ? 'text-lg font-semibold text-white' : 'text-base font-medium text-white transition-all duration-200'}`}>
                                        {concept.title}
                                    </span>
                                </button>
                            );
                    ...
                    {/* Right: Giant Typographic Display Panel (No borders, pure text) */}
                    <div className="w-full lg:w-2/3 min-h-[300px] flex items-center relative">
                        {/* Content */}
                        <div className="w-full max-w-3xl">
                            <div className="w-12 h-12 rounded-xl bg-[#034EA2] text-white flex items-center justify-center mb-6">
                                <div className="w-6 h-6">
                                    {activeConcept.icon}
                                </div>
                            </div>
                            <h2 className="text-2xl lg:text-3xl font-inter font-semibold tracking-tight text-white mb-4 leading-tight">
                                {activeConcept.title}.
                            </h2>
                            <p className="text-sm lg:text-base font-inter text-gray-400 leading-relaxed max-w-2xl">
                                {activeConcept.description}
                            </p>
                        </div>

                        {/* Extremely subtle background light mapping to the active item */}
                        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-br from-[#034EA2]/10 to-transparent rounded-full blur-[100px] opacity-30 pointer-events-none transition-all duration-1000"></div>
                    </div>
                </div>
            ...
            <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
                <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-gradient-to-bl from-[#034EA2]/5 via-white/5 to-transparent rounded-full blur-3xl opacity-50" />
            </div>
