...
const LogoCarousel = ({ logos }) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto overflow-hidden py-8 md:py-12 px-4">
      {/* Glass container */}
      <div className="relative bg-white/[0.03] backdrop-blur-2xl border border-white/[0.06] rounded-3xl p-6 md:p-8 shadow-2xl shadow-black/10 overflow-hidden">
        {/* Subtle gradient overlay */}
        <div className="absolute inset-0 bg-gradient-to-r from-white/[0.02] via-transparent to-white/[0.02] rounded-3xl"></div>
        
        {/* Inner glass effect */}
        <div className="relative bg-white/[0.02] backdrop-blur-sm border border-white/[0.04] rounded-2xl p-4 md:p-6 overflow-hidden">
          {/* Fade masks */}
            ...
            {duplicatedLogos.map((logo, index) => (
              ...
              >
                ...
                <div className="relative bg-white/[0.04] backdrop-blur-sm border border-white/[0.08] rounded-2xl p-6 transition-all duration-500 hover:bg-white/[0.08] hover:border-white/[0.15] hover:shadow-xl hover:shadow-white/5 hover:scale-105">
                  ...
                  src={logo}
                  alt={`Cliente ${index + 1}`}
                  className="relative w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 object-contain filter grayscale hover:grayscale-0 transition-all duration-500 group-hover:brightness-110"
                />
                </div>
              </div>
            ))}
